...
describe("Categories CRUD", () => {
	...
	describe("PUT /categories/:id - Atualizar categoria", () => {
		it("deve atualizar uma categoria com sucesso (admin)", async () => {
			...
			const body = JSON.parse(response.body);
			expect(body.category.name).toBe("Test Category Updated");
			expect(body.category.slug).toBe("test-category-updated");
			expect(body.category.description).toBe("Descrição atualizada");
		...
		it("deve atualizar apenas o nome sem alterar description", async () => {
			...
			const body = JSON.parse(response.body);
			expect(body.category.name).toBe("Test Category Final Name");
			expect(body.category.slug).toBe("test-category-final-name");
			// Description deve permanecer a mesma
			expect(body.category.description).toBe("Descrição original");
		});
		...
		it("deve atualizar status active da categoria", async () => {
			...
			const body = JSON.parse(response.body);
			expect(body.category.active).toBe(false);
		});
