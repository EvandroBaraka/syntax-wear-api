...
describe("Products CRUD", () => {
    ...
    describe("PUT /products/:id - Atualizar produto", () => {
        it("deve atualizar um produto com sucesso (admin)", async () => {
            ...
            expect(body.product.name).toBe("Test Product Updated");
            expect(body.product.slug).toBe("test-product-updated");
            expect(Number(body.product.price)).toBe(149.99);
        ...
        it("deve atualizar apenas campos específicos", async () => {
            ...
            const body = JSON.parse(response.body);
            expect(body.product.stock).toBe(50);
            // Nome deve permanecer o mesmo
            expect(body.product.name).toBe(mockProduct.name);
        });
    });
